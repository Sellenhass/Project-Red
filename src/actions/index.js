...
export function getDailyPhoto() {
  return (dispatch) =>
    api.getDailyPhoto().then((response) =>
      dispatch({
...
export function getAsteroidsList(date) {
  return (dispatch) =>
    api.getAsteroidsList(date).then((response) =>
      dispatch({
...
export function getExactBeer(beerId) {
  return (dispatch) =>
    api.getExactBeer(beerId).then((response) =>
      dispatch({
        type: GET_EXACT_BEER,
...
export function searchBeers(beerName) {
  return (dispatch) =>
    api.searchBeers(beerName).then((response) =>
      dispatch({
        type: SEARCH_BEERS,
...
export function applyFilters(abvValue, ebcValue, ibuValue) {
  return (dispatch) =>
    api.applyFilters(abvValue, ebcValue, ibuValue).then((response) =>
      dispatch({
        type: APPLY_FILTERS,
